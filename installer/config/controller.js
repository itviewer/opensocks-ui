...
Controller.prototype.onInstallationFinished = function ()
{
    ...
    if (installer.isInstaller() && installer.status === QInstaller.Success) {
        if (systemInfo.kernelType === "linux") {
            installer.executeDetached("@TargetDir@/bin/opensocks-ui");
        } else if (systemInfo.kernelType === "winnt") {
            installer.executeDetached("@TargetDir@/opensocks-ui.exe");
        } else if (systemInfo.kernelType === "darwin") {
            installer.executeDetached("@TargetDir@/OpenSocks.app/Contents/MacOS/OpenSocks");
        }
